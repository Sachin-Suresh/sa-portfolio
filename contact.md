---
layout: default
title: "Contact"
---

<section class="section" style="max-width: 720px; margin: auto;">
  <div class="container">

    <div class="content mb-5">
      <h1>Contact Me</h1>
      <p class="subtitle is-5">Let’s connect and create something impactful.</p>
    </div>

    <div class="content mb-5">
      <ul style="list-style: none; padding-left: 0;">
        <li style="margin-bottom: 0.5rem;">
          <img src="../assets/icons/email.svg" alt="Email icon" style="width: 18px; margin-right: 8px; vertical-align: middle;">
          <strong>Email:</strong> <a href="mailto:sach7411@gmail.com">sach7411@gmail.com</a>
        </li>
        <li style="margin-bottom: 0.5rem;">
          <img src="../assets/icons/linkedin.svg" alt="LinkedIn icon" style="width: 18px; margin-right: 8px; vertical-align: middle;">
          <strong>LinkedIn:</strong> <a href="https://www.linkedin.com/in/sachin-mca/" target="_blank">linkedin.com/in/sachin-mca</a>
        </li>
        <li style="margin-bottom: 0.5rem;">
          <img src="../assets/icons/github.svg" alt="GitHub icon" style="width: 18px; margin-right: 8px; vertical-align: middle;">
          <strong>Portfolio:</strong> <a href="https://sachin-suresh.github.io/sa-portfolio/" target="_blank">sachin-suresh.github.io/sa-portfolio/</a>
        </li>
      </ul>
    </div>

    <p class="has-text-grey-dark">
      If you’d like to collaborate, hire me, or request custom samples — I’m open to remote roles and documentation consulting.
    </p>

  </div>
</section>
